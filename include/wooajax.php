...
add_action('wp_head','AlitaStore_woo_ajaxurl');
function AlitaStore_woo_ajaxurl() {
?>
...
function product_quickview() {
	...
	<div class="woocommerce product">
		<div class="product-images">
			...
			<?php $name_p =  $product->get_title();?>
			
			<div class="main-image images"><img src="<?php echo esc_attr($image_link);?>" alt="<?php echo esc_attr($name_p);?>" /></div>
			<?php
		...
		<div class="product-info">
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($AlitaStore_opt['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<div class="social-sharing"><?php AlitaStore_product_sharing(); ?></div>
		</div>
	</div>
	<?php
	die();
...
function get_cartinfo() {
	...
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'AlitaStore'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	...
	?>
	<h3><?php esc_html_e('Product is added to cart', 'AlitaStore');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo get_permalink( wc_get_page_id( 'cart' ) );?>"><?php esc_html_e('View Cart', 'AlitaStore');?></a>
	</div>
	<?php
	die();
}
